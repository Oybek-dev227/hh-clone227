...
export default function MyJobCard({item,selectedJob,setSelectedJob}) {
    ...
    // useRouter() - chaqirishimizdan maqsaq buning ichida bir funksiya bor uni ishlatsak boshqa pagega utishimizga yordam beradi
    const router = useRouter()

    const handlePress =()=>{
        // push -degani boshqa sahifaga otvoradi
        ...
        <TouchableOpacity style={styles.container(selectedJob, item?.job_id)} onPress={()=>handlePress()}>
            <View style={styles.employerWrapper}>
                <View style={styles.employerHeader}>
                    <Image source={{uri: item?.employer_logo ? item.employer_logo : "https://avatars.mds.yandex.net/i?id=7e02defb348bc1b24d063cc78875bcb87b586fc0-10869844-images-thumbs&n=13" }} style={styles.employerLogo}/>
                    <Text style={styles.employerTitle}>{item?.employer_name.length >= 20 ? item?.employer_name.slice(0, 20) + "..." : item?.employer_name}</Text>
                </View>
